...
export default {
    ...
    methods: {
        draw_graph: function() {
            if (this.id === "hist"){
                var data_color_values = this.$store.state.topology.data_color_values;
                var statistic_value = this.$store.state.topology.statistic_value;
            }else{
                var data_color_values = this.$store.state.topology.data_color_values_col2;
                var statistic_value = this.$store.state.topology.statistic_value_col2;
            }
            ...
            var min_x = Math.min.apply(null, data_color_values)
            var max_x = Math.max.apply(null, data_color_values)
            ...
            let interpolation_hsv = function(value, q_min, q_max, val_min, val_max) {
                var i = (value - val_min) / (val_max - val_min);
                var h = i * 0.25 + q_min;
                var hsv = d3.hsl((1-h)*240, 0.7, 0.5);
                return hsv+"";
            }
