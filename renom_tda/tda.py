...
class TopologyCore(object):
    ...
    def color(self, target, color_method="mean", color_type="rgb", normalized=False):
        """
        colored by target
        """
        if target is None:
    ...
    def output_csv_from_node_id(self, filename, node_ids=[], target=None):
        """
        output file
        """
        if len(node_ids) > 0:
            number_data, number_data_columns = self._concatenate_target(target)
            data_index = self._data_index_from_node_id(node_ids)
            extract_data = number_data[data_index]
            columns = ','.join(number_data_columns)
            np.savetxt(filename, extract_data, delimiter=",", header=columns)
